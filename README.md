...
| `ferret home` | local | ✓ | — | Measurement scoreboard: top routines by burn, priced single-call waste, burn-delta since the last fix (the bare-ferret default). |
...
| `ferret floor` | local | — | `--root` `--format` `--limit` `--max-bytes` `--rules-global` `--rules-project` | Per-session fixed load floor: SessionStart hook + tool/skill roster bytes paid before dk's first real turn, as a share of total session bytes (rule files measured from disk). |
